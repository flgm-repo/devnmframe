	...
	$.fn.extend({
		infiniteCarousel: function(options)
		{
			var defaults =
			{
				...
				thumbnailFontSize: '100%',
				easeLeft: 'swing',
				easeRight: 'swing',
				imagePath: '/images/infiniteloop/',
				inView: 1,
				padding: '0px',
				advance: 1,
				showControls: false,
		...
		var options = $.extend(defaults, options);
			...
			if(o.displayThumbnails)
			{
				...
				$('#thumbs'+randID+' div.thumb:not(:first)').css({'opacity':.45}); // makes all thumbs 65% opaque except the first one


				$('#thumbs'+randID+' div.thumb').hover(function(){$(this).animate({'opacity': 1},150)},function(){if(viewable[0]!=this.id.split('_')[1]) $(this).animate({'opacity':.45},250)}); // add hover to thumbs
